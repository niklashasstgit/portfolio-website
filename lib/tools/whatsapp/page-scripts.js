...
function installScraper() {
  ...
  function findChatRows(pane) {
    ...
    /* If any row carries a span[title], this build still sets titles, so the
       loose name heuristic must stay off - it is what adopts pseudo-rows. */
    var loose = qsa('span[title]', pane).length === 0;

    var best = { rows: [], strategy: 'none', loose: loose };
    for (var i = 0; i < strategies.length; i++) {
      ...
      for (var j = 0; j < rows.length; j++) {
        ...
        seen.push(rows[j]);
        if (isNonChatRow(rows[j])) continue;
        if (rowName(rows[j], loose).name) named.push(rows[j]);
      }
      if (named.length > best.rows.length) {
        best = { rows: named, strategy: strategies[i][0], loose: loose };
      }
    }
    return best;
  }
  ...
  /* Pull the chat title (and the preview line) out of one row. The title
     ...
     stop setting it. */
  /*
   * #pane-side contains rows that are not conversations: the "Archived" folder
   * entry above the list being the important one. It has no span[title], so the
   * last-resort name heuristic below used to adopt its label as a chat name -
   * and opening it navigates into a sub-view, which both destroys the page
   * context and leaves every later chat genuinely absent from the list.
   */
  var NON_CHAT_LABEL = /^(archived|archiviert|archivados|archiv[eé]es?|archiviate|arquivadas?|gearchiveerd|arkiverade|arkiverte|arkistoidut|zarchiwizowane|arhivate|\u0410\u0440\u0445\u0438\u0432)\b/i;

  function isNonChatRow(row) {
    if (qs('[data-testid*="archived" i], [data-icon*="archived" i], [aria-label*="archived" i]', row)) return true;
    var label = (nodeText(row) || '').trim();
    if (!label) return true;
    /* the archived entry is a short label, optionally followed by a count */
    if (label.length < 40 && NON_CHAT_LABEL.test(label)) return true;
    return false;
  }

  function rowName(row, allowLooseName) {
    var name = '';
    var preview = '';
    ...
    if (!name && allowLooseName) {
      /* Last resort, used only when NO row in the pane carries a span[title]
         (i.e. the build genuinely stopped setting it). Applying it per-row is
         what let the "Archived" entry masquerade as a chat. */
  ...
  WCE.readChatListPage = function () {
    ...
    for (var i = 0; i < rows.length; i++) {
      var row = rows[i];
      var named = rowName(row, found.loose);
      var name = named.name;
  ...
  /* If a click has taken the side panel into a sub-view (the archived folder,
     say), get back to the main chat list. Only acts when a sub-view is
     actually showing, so it can never navigate away from the normal list. */
  WCE.leaveSubView = function () {
    var side = qs('#side') || document;
    var header = qs('header', side);
    var headerText = header ? (nodeText(header) || '').trim() : '';
    var inSubView = !!qs('[data-testid*="archived" i]', side) ||
                    (headerText.length < 40 && NON_CHAT_LABEL.test(headerText));
    if (!inSubView) return false;

    var rx = /^(back|zur[uü]ck|atr[aá]s|retour|indietro|voltar|terug)$/i;
    var btns = qsa('button[aria-label], [role="button"][aria-label], [data-icon="back"]', side);
    for (var i = 0; i < btns.length; i++) {
      var lab = (btns[i].getAttribute('aria-label') || '').trim();
      if (rx.test(lab) || btns[i].getAttribute('data-icon') === 'back') {
        (btns[i].closest('button') || btns[i]).click();
        return lab || 'back';
      }
    }
    return false;
  ...
  WCE.locateChatRow = function (name) {
    ...
    var rows = found.rows;
    for (var i = 0; i < rows.length; i++) {
      var candidates = [rowName(rows[i], found.loose).name];
  ...
  WCE.diagnose = function () {
    ...
    if (pane) {
      ...
      report.chatList = {
        ...
        firstRowName: found.rows[0] ? peek(rowName(found.rows[0], found.loose).name) : null
      };
      var sc = findScroller(pane) || pane;
