...
/**
 * PIN gate for /tools. Deliberately posts to the same /api/admin/login as the
 * admin console: one password, one session cookie, both areas.
 */
export default function ToolsLogin({ configured }: { configured: boolean }) {
  ...
  return (
    ...
    >
      ...
      <h1 className="mt-2 text-lg font-semibold text-fg">Tools</h1>
      <p className="mt-1 text-sm text-fg-muted">
        {configured
          ? "Enter your PIN to open the toolbox."
          : "No PIN is configured. Set ADMIN_PIN in the environment, then reload."}
      </p>
      <input
        ref={inputRef}
        type="password"
